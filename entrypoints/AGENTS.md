...
Before task-specific work, load the project harness in `cow-harness/`.
...
8. `cow-harness/core/runbooks.md` when the task matches a runbook

The OMX runtime owns orchestration. The `cow-harness/` layer owns project boundaries, artifact contracts, verification gates, and migration portability.
Default harness routes are mandatory baselines. If the user names a skill or tool, treat it as additive to `cow-harness/core/routing.md` unless the user explicitly says to skip, disable, or only use a different route.
When this AGENTS.md is regenerated, preserve this overlay or reapply it from `cow-harness/README.md`.
